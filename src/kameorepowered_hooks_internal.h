...
// relocated by the update and are absent from the patched image. The combat and
// audio-volume hooks ARE re-derived (see kameorepowered_hooks_combat.cpp /
// kameorepowered_hooks_audio.cpp / kameorepowered_tu_overrides.toml) and call
// their relocated guest functions directly, not through this shim. What still
// goes through this stub on TU: the DLC swap (intentionally disabled — the update
// doesn't need it) and the string-table language reload (its patched-image
// signature/globals aren't fully re-derived yet). Those call sites are compiled
// out to keep the project linking (the argument expression is still evaluated so
// locals stay "used"). TODO: finish the language-reload RE (see tu-audio).
#ifdef KAMEO_TU
